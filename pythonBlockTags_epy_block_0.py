...
class blk(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
    ...
    def work(self, input_items, output_items):
        "Search for samples above threshold"
        for index in range(len(input_items[0])):
            # We found detected a value above threshold, set the Tag
            ...
            # Increment timer after Tag has been set
            if (self.readyForTag == False):
                self.timer += 1
            # Once timer is finished, we are ready to detect again
